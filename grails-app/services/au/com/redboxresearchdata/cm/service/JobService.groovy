...
import au.com.redboxresearchdata.cm.domain.CurationJob
import au.com.redboxresearchdata.cm.domain.CurationJobItems
                ...
                for (item in json) {
                    ...
                    item.required_identifiers.each { reqId ->
                        ...
                        if (!reqId.identifier_type) {
                            ...
                            log.debug("identifier type parsed: " + reqId.identifier_type)
                        }
                        ...
                        for (idProvider in idProviders) {
                            if (idProvider == reqId.identifier_type) {
                                log.debug("required identifier type found in list of enabled id providers...")
                                // we got a match, proceed
                                hasProvider = true
                                String metadata = reqId?.metadata ? JsonOutput.toJson(reqId.metadata) : "{}"
                                // setting to empty Object as per https://qcifltd.atlassian.net/wiki/display/REDBOX/Identity+Providers#IdentityProviders-Result
                    ...
                    if (!jobItem.validate()) {
                        log.debug("job items failed validation...")
                        jobItem.errors.each {
                            log.error it
                        }
                    } else {
                        log.debug("saving job item...")
                        jobItem.save()
